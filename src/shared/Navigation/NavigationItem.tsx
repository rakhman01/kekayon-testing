...
import TemplatesDropdown from "@/app/(client-components)/(Header)/TemplatesDropdown";
...
export interface NavItemType {
  ...
  type?: "dropdown" | "megaMenu" | "none" | 'superMenu';
}
...
const NavigationItem: FC<NavigationItemWithRouterProps> = ({ menuItem }) => {
  ...
  const renderMegaMenuNavlink = (item: NavItemType) => {
    const IconComponent = item?.icon.src;
    ...
    return (
      ...
      key={item.id}
        ...
        rel="noopener noreferrer"
        className="col-span-1 font-normal text-neutral-6000 dark:text-neutral-300 px-4 rounded-md hover:text-neutral-700 hover:bg-neutral-100 dark:hover:bg-neutral-800 dark:hover:text-neutral-200 "
        href={item.href || ""}
      >

        <div className="flex items-start justify-start">
          <div className="w-3 h-3">
            {IconComponent && <img src={IconComponent} alt="Icon" className="w-3 h-3 text-primary-6000" />}

          </div>
          <div className="w-auto text-start ml-2">
            <p className="text-xs font-normal">{item.name}</p>
            <p className="text-[8px] font-thin text-gray-500">{item.subName}</p>
          </div>
        </div>
        ...
        {() => (
          <>
            ...
            >
              <Popover.Panel
                static
                className="sub-menu will-change-transform absolute transform z-10 w-96 top-full left-0"
              >
      ...
      >
        ...
        <div className="flex items-start justify-start">
          <div className="w-6 h-6">
            <IconComponent className="w-6 h-6 text-primary-6000" />
          </div>
          <div className="w-auto text-start ml-2">
  ...
  // ===================== MENU SUPER MENU =====================
  const renderSuperItem = (item: NavItemType) => {
    return (
      <Link
        rel="noopener noreferrer"
        className="inline-flex items-center text-sm xl:text-base font-normal text-neutral-700 dark:text-neutral-300 py-2 px-4 xl:px-5 rounded-full hover:text-neutral-900 hover:bg-neutral-100 dark:hover:bg-neutral-800 dark:hover:text-neutral-200"
  ...
  const renderSuperMenu = (menuDropdown: any) => {
    const isHover = menuCurrentHovers.includes(menuDropdown.id);
    return (
      <Popover
        as="li"
        className={`menu-item flex items-center menu-dropdown ${menuDropdown.isNew ? "menuIsNew_lv1" : ""
          }`}
        onMouseEnter={() => onMouseEnterMenu(menuDropdown.id)}
        onMouseLeave={() => onMouseLeaveMenu(menuDropdown.id)}
      >
        {() => (
          <>
            <div>{renderSuperItem(menuDropdown)}</div>
            <Transition
              as={Fragment}
              show={isHover}
              ...
              leaveTo="opacity-0 translate-y-1"
            >
              <Popover.Panel className="absolute z-20 top-full min-w-full inset-x-0 left-0 ">
                <div className="bg-white dark:bg-neutral-900 shadow-lg max-h-screen overflow-auto">
                  <div className="container">
                    <div className="flex text-sm border-t border-slate-200 dark:border-slate-700 py-14 pb-48">
                      <div className="flex-1 grid grid-cols-6 gap-6 xl:gap-8 pr-6 xl:pr-8">
                        {NAVIGATION_DEMO_3.map((item, index) => (
                          <div key={index} className="col-span-2">
                            <p className="font-medium text-slate-900 dark:text-neutral-200">
                              {item.name}
                            </p>
                            <ul className="grid col-span-4 space-y-4 mt-4">
  ...
  switch (menuItem.type) {
    ...
    case "superMenu":
      return renderSuperMenu(menuItem)
    default:
      return (
